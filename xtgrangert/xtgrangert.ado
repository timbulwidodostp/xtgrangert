...
syntax varlist(numeric ts) [if][in][,lags(integer 1) ///
	...
	rm0				/// set cross-sections with missing X'X to zero
	]
...
if (`maxlags'!=0){
	if "`test0'" == "" {
		mata: test01("`depvar'","`indeps'","`idvar' `tnew'",`maxlags',"`rm0'"!="")
	}
	else {
		mata: test0("`depvar'","`indeps'",`t',`n',`maxlags',"`rm0'"!="")
	}
...
`trace' mata: test1("`depvar'","L(1/`lags').(`indeps')","L(1/`lags').`depvar'","`idvar' `tnew'","`touse'",`n',`t',`lags',"`het'","`dfc'",`bootstrapdraws',"`nonoverlap'"!="",`unbalanced',"`rm0'"!="")
...
mata:
void test0 (string scalar depvar,string scalar indeps, numeric scalar t, numeric scalar n,numeric scalar l,numeric scalar rm)
{
...
mata:
void test01 (string scalar depvar,string scalar indeps, string scalar idt ,numeric scalar l,numeric scalar rm0)
{
	z1=st_data(.,depvar,.) //the transfer (t*n)*1 matrix
	z2=st_data(.,indeps,.)
	...
	if (Tmin <= l) {
		...
		for (ll=1;ll<=l;ll++) {
			...
			for (i=1;i<=n;i++) {
				...
				RSSi = (yi-xi*tempbeta)'*Mi*(yi-xi*tempbeta)
				if (rm0:==1) {
					_editmissing(RSSi,0)
				}
...
mata:
void test1(string scalar depvar,string scalar indeps, string scalar depvarlag,string scalar idtn, string scalar tousen, numeric scalar n, numeric scalar t, numeric scalar p,string scalar het,string scalar dfc,real scalar bootdraws , numeric scalar overlap, numeric scalar unbalanced,numeric scalar rm0)
{
	...
	beta = estBeta(z1,z2,z3,index,(1::n),p,RSS=0,b=.,dof=0,overlap,rm0)
	...
	if (bootdraws == 0) {
		...
		for (r = 1;r<=bootdraws-1;r++) {
			...
			beta_rr = estBeta(z1,z2,z3,index,(floor(n:*runiform(n,1) :+ 1)),p,tmp1=0,tmp2=.,tmp3=.,overlap,rm0)
			beta_r[r,.] = beta_rr'
...
mata:
	function estBeta(real matrix y, real matrix x, real matrix Ly, real matrix index,  real matrix sel,real scalar p,real scalar RSS, real matrix b,real scalar dof,numeric scalar overlap_opt, numeric scalar rm0)
	{
		...
		for (i=N;i>0;i--) {		
			...
			///i, ii, mid,ti
			/// full panel
			ols_inner(yi,xi,Lyi,xx,xy,RSS,rm0)

			/// First part of panel
			ols_inner(yi[|1,. \ mid-overlap,.|],xi[|1,. \ mid-overlap,.|],Lyi[|1,. \ mid-overlap,.|],xx_f,xy_f,tmp=.,rm0)

			// Second Part of panel
			ols_inner(yi[|mid+1,. \ .,.|],xi[|mid+1,. \ .,.|],Lyi[|mid+1,. \ .,.|],xx_u,xy_u,tmp=.,rm0)
		}
...
mata:
	function ols_inner(real matrix y, real matrix x, real matrix z, xx,xy ,RSS,rm0)
	{
		...
		RSSi = (y-x*tempbeta)'*Mi*(y-x*tempbeta)
		if (rm0:==1) {
			_editmissing(tmpxx,0)
			_editmissing(tempxy,0)
			_editmissing(RSSi,0)
		}
